...
import { AjoutPersonneLieComponent } from './ajout-personne-lie/ajout-personne-lie.component';

@Component({
    selector       : 'personne-lie',
    templateUrl    : './personne-lie.component.html',
    encapsulation  : ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class PersonneLierComponent implements OnInit
{
    ...
    signataireColumns: string[] = ['prenom','nom','liaison','dateCreated'];
    dialogRef: any;
    ...
    ajoutActionnaire(): void {
        this.dialogRef = this._matDialog.open(AjoutPersonneLieComponent, {
          autoFocus: true,
          width: '43rem',
     ...
     getPersonne() {
      this.isLoading = true;
      this.CoreService.list('personne-lie',this.offset,this.pageSize,).subscribe((resp) => {
        if (resp[this.constantes.RESPONSE_CODE] === this.constantes.HTTP_STATUS.SUCCESSFUL) {
          this.isLoading = false;
           this.signataireDataSource.data = resp[this.constantes.RESPONSE_DATA].filter(item => item?.client?.id == this.paramsId);
          this._changeDetectorRef.markForCheck();
        } else {
          this.isLoading = false;
        }
