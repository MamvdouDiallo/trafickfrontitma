...
@Component({
    ...
    styleUrls: ['./ajout-demande-credit.component.scss']
})
export class AjoutDemandeCreditComponent implements OnInit {
    ...
    constructor(public matDialogRef: MatDialogRef<AjoutDemandeCreditComponent>,
    ...
    addItems() {
        this.snackbar.showConfirmation('Voulez-vous vraiment ajouter cette demande de crédit ?').then((result) => {
            if (result['value'] == true) {
                ...
                value['dateCreated'] = this.dateComptable;
                    ...
                    this.coreService.addItem(value, 'demandes-credits').subscribe(
                        (resp) => {
                            if (resp) {
                                ...
                                this.snackbar.openSnackBar('Demande ajoutée avec succés', 'OK', ['mycssSnackbarGreen']);
                            } else {
                                this.loader = false;
